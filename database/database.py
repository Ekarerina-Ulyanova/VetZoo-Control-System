...
class Database:
    ...
    def migrate_tables(self):
        try:
            ...
            if "is_scheduled" not in columns:
                ...
                self.cursor.execute(
                    "ALTER TABLE examinations ADD COLUMN is_scheduled INTEGER DEFAULT 0"
                )
                self.conn.commit()
            ...
            if "is_scheduled" not in columns:
                ...
                self.cursor.execute(
                    "ALTER TABLE vaccinations ADD COLUMN is_scheduled INTEGER DEFAULT 0"
                )
                self.conn.commit()
    ...
    def add_animal(
        ...
        if arrival_date:
            try:
                ...
                raise ValueError(
                    "Неверный формат даты прибытия. Используйте ГГГГ-ММ-ДД"
                )
        ...
        if birth_date:
            try:
                ...
                if (
                    arrival_date
                    and birth > datetime.strptime(arrival_date, "%Y-%m-%d").date()
                ):
                    raise ValueError("Дата рождения не может быть позже даты прибытия")
            except ValueError:
                raise ValueError(
                    "Неверный формат даты рождения. Используйте ГГГГ-ММ-ДД"
                )
    ...
    def update_animal_status(self, animal_id, status):
        ...
        self.cursor.execute(
            "UPDATE animals SET health_status = ? WHERE id = ?", (status, animal_id)
        )
        self.conn.commit()
        ...
        if birth_date is not None:
            if birth_date:
                try:
                    ...
                    check_arrival = (
                        arrival_date if arrival_date is not None else current[3]
                    )
                    if (
                        check_arrival
                        and birth > datetime.strptime(check_arrival, "%Y-%m-%d").date()
                    ):
                        raise ValueError(
                            "Дата рождения не может быть позже даты прибытия"
                        )
                except ValueError:
                    raise ValueError(
                        "Неверный формат даты рождения. Используйте ГГГГ-ММ-ДД"
    ...
    def get_animal_examinations(self, animal_id):
        self.cursor.execute(
            "SELECT * FROM examinations WHERE animal_id = ? ORDER BY examination_date DESC",
            (animal_id,),
        )
        return self.cursor.fetchall()
    ...
    def complete_examination(self, exam_id):
        ...
        self.cursor.execute(
            "UPDATE examinations SET is_scheduled = 0 WHERE id = ?", (exam_id,)
        )
        self.conn.commit()
        return self.cursor.rowcount > 0
        ...
        if vaccination_date:
            try:
                ...
                if not is_scheduled and vac_date > today:
                    raise ValueError(
                        "Дата проведенной прививки не может быть в будущем"
                    )
            except ValueError:
                raise ValueError(
                    "Неверный формат даты прививки. Используйте ГГГГ-ММ-ДД"
                )
        ...
        if next_due_date:
            try:
                ...
                if (
                    vaccination_date
                    and next_date
                    <= datetime.strptime(vaccination_date, "%Y-%m-%d").date()
                ):
                    raise ValueError(
                        "Дата следующей прививки должна быть позже даты текущей"
                    )
            except ValueError:
                raise ValueError(
                    "Неверный формат даты следующей прививки. Используйте ГГГГ-ММ-ДД"
                )
    ...
    def complete_vaccination(self, vacc_id):
        ...
        self.cursor.execute(
            "UPDATE vaccinations SET is_scheduled = 0 WHERE id = ?", (vacc_id,)
        )
        self.conn.commit()
        return self.cursor.rowcount > 0
    ...
    def add_diet(
        ...
        if not diet_name or not food_type or not quantity:
            raise ValueError(
                "Название рациона, тип корма и количество обязательны для заполнения"
            )
        ...
        if end_date:
            try:
                ...
                if (
                    start_date
                    and end <= datetime.strptime(start_date, "%Y-%m-%d").date()
                ):
                    raise ValueError("Дата окончания должна быть позже даты начала")
            except ValueError:
                raise ValueError(
                    "Неверный формат даты окончания. Используйте ГГГГ-ММ-ДД"
                )
        ...
        """,
            (
                ...
                food_type[:100],
                quantity[:50],
                schedule[:200] if schedule else None,
                start_date,
                end_date,
            ),
        )
    ...
    def get_animal_diets(self, animal_id):
        self.cursor.execute(
            "SELECT * FROM diets WHERE animal_id = ? ORDER BY start_date DESC",
            (animal_id,),
        )
        return self.cursor.fetchall()
    ...
    def update_user_credentials(
        self, user_id, new_username=None, new_password_hash=None
    ):
        if new_username:
    ...
    def update_user_fullname(self, user_id, new_full_name):
        ...
        self.cursor.execute(
            "UPDATE users SET full_name = ? WHERE id = ?", (new_full_name, user_id)
        )
        self.conn.commit()
        return self.cursor.rowcount > 0
