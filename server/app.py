...
def change_credentials():
    ...
    success, message = db.update_user_credentials(
        current_user.id, new_username, new_password_hash
    )
    ...
    try:
        ...
        for exam in exams:
            try:
                ...
                print(
                    f"Осмотр ID {exam[0]}, is_scheduled: {is_scheduled}, дата: {exam_date_str}"
                )
                    ...
                    if vaccine[5]:
                        try:
                            ...
                            if days_until >= -30:
                                ...
                                if days_until < 0:
                                    description += (
                                        f" ⚠️ просрочена на {abs(days_until)} дн."
                                    )
                                ...
                                result.append(
                                    {
                                        ...
                                        "days_until": days_until,
                                        "status": (
                                            "upcoming" if days_until > 0 else "overdue"
                                        ),
                                    }
                                )
                        except Exception as e:
...
def get_health_status_report():
    try:
        ...
        for animal in animals:
            ...
            species_stats[species]["statuses"][status] = (
                species_stats[species]["statuses"].get(status, 0) + 1
            )
        ...
        for animal in animals:
            ...
            if vaccines:
                ...
                for vaccine in vaccines:
                    ...
                    report["vaccines_by_type"][vaccine_name] = (
                        report["vaccines_by_type"].get(vaccine_name, 0) + 1
                    )

                    is_scheduled = (
                        vaccine[6] if len(vaccine) > 6 and vaccine[6] == 1 else 0
                    )
                    ...
                    if not is_scheduled:
                        try:
                            ...
                            report["vaccinations_by_month"][month_key] = (
                                report["vaccinations_by_month"].get(month_key, 0) + 1
                            )
                        except:
                            pass
                ...
                report["animals_without_vaccinations"].append(
                    {"id": animal_id, "name": animal_name, "species": animal_species}
                )

        if animals:
            report["vaccination_coverage"] = round(
                (animals_with_vaccines / len(animals)) * 100, 2
            )
...
def get_examinations_report():
    try:
                ...
                for exam in exams:
                    ...
                    try:
                        exam_date = datetime.strptime(
                            exam[2].split()[0], "%Y-%m-%d"
                        ).date()

                        report["examinations_by_vet"][vet] = (
                            report["examinations_by_vet"].get(vet, 0) + 1
                        )
                        report["common_diagnoses"][diagnosis] = (
                            report["common_diagnoses"].get(diagnosis, 0) + 1
                        )
                        ...
                        if not is_scheduled:
                            ...
                            report["examinations_by_month"][month_key] = (
                                report["examinations_by_month"].get(month_key, 0) + 1
                            )
        ...
        sorted_diagnoses = sorted(
            report["common_diagnoses"].items(), key=lambda x: x[1], reverse=True
        )[:10]
